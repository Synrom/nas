...
from dataset.utils import cifar_label2name
...
class Monitor:
    ...
    def add_hooks(self):
        ...
        for module in self.model.modules():
            if isinstance(module, nn.ReLU):
                self.forward_hooks[module] = module.register_forward_hook(self.plot_inputs(f"{i}th ReLU Layer Inputs", i-1))
                self.backward_hooks[module] = module.register_full_backward_hook(self.plot_gradients(f"{i}th ReLu Layer Gradients", i-1))
                i+=1
    ...
    def first_batch(self, imgs: torch.Tensor, X: torch.Tensor, Y: torch.Tensor, loss: float):
        ...
        for i in range(num_samples):
            ...
            axs[i, 0].set_title(f"Label: {self.label2name[Y[i].item()]}")
            axs[i, 0].axis('off')
    ...
    def input_dependent_baseline(self, model: nn.Module, criterion: nn.Module):
        _, X, Y = self.test_batch
        X, Y = X.to(self.device), Y.to(self.device)
        out = model(X)
        loss_real = criterion(out, Y)

        zeroes = torch.zeros_like(X)
        out_zeroes = model(zeroes)
        loss_zeroes = criterion(out_zeroes, Y)
    ...
    def test_data_sharing_inbetween_batch(self, model: nn.Module, X: torch.Tensor):
        ...
        assert input.grad[i].sum() != 0.0
        for idx in range(input.shape[0]):
            if idx == i:
                continue
            assert input.grad[idx].sum() == 0.0
        model.zero_grad()
    
    def overfit_single_batch(self, model: nn.Module, X: torch.Tensor, Y: torch.Tensor, criterion: nn.Module, optimizer: optim.Optimizer, n: int = 1000):
        # clone everything
